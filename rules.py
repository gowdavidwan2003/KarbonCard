...
def total_revenue(data: dict, financial_index):
    ...
    try:
        ...
        if financial_index < 0 or financial_index >= len(financials):
            raise IndexError("Financial index is out of range.")
        ...
        print(f"Error occurred while fetching net revenue: {e}")
        return 0.0
...
def total_borrowing(data: dict, financial_index):
    ...
    long_term_borrowings = financial_data['bs']['liabilities'].get('long_term_borrowings', 0)
    short_term_borrowings = financial_data['bs']['liabilities'].get('short_term_borrowings', 0)
    
    
    total_borrowings = long_term_borrowings + short_term_borrowings

    total_revenue1 = total_revenue(data, financial_index)
    

    if total_revenue1 == 0:
        return None  
    
    borrowing_ratio = total_borrowings / total_revenue1
...
def iscr(data: dict, financial_index):
    ...
    try:
        ...
        pbit = line_items.get("profit_before_interest_and_tax", 0.0)
        depreciation = line_items.get("depreciation", 0.0)
        interest = line_items.get("interest", 0.0)

   
        iscr_value = (pbit + depreciation + 1) / (interest + 1)
        
        return iscr_value
    
    except (KeyError, IndexError, TypeError, ValueError) as e:
        print(f"Error occurred while calculating ISCR: {e}")
        return 0.0
